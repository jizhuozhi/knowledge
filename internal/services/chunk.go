...
func (s *ChunkService) structureAwareSectionChunk(doc *models.Document, blocks []mdBlock, strategy *IndexStrategy) ([]models.Chunk, error) {
	...
	for _, block := range blocks {
		if block.Type == mdBlockHeading {
			// New heading: flush current section (unless it's huge, handle below)
			if currentLength > 0 {
				flushSection()
			}
			currentHeader = block.Content
			currentBlocks = []mdBlock{block}
			currentLength = len(block.Content)
			continue
		}
		...
		if currentLength+blockLen > chunkSize*2 && len(currentBlocks) > 0 {
			flushSection()
			// Keep the current header context
		}
